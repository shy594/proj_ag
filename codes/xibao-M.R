...
#03、写函数
{
  volcan <- function(f_Inputs, f_Output = "1111"){
    ress <- f_Inputs
    ress$cuts <- "theOthers"
    ress$cuts[ress$avg_log2FC >  log2(1.5) & ress$p_val_adj <0.05] = "Upup__Features"
    ress$cuts[ress$avg_log2FC < -log2(1.5) & ress$p_val_adj <0.05] = "Down__Features"
...
#06、细胞类型注释 [自动注释]
{
  ...
  p___03 <- Seurat::DimPlot(objs_8, reduction = "umap",
                            ...
                            cols = c(ggsci::pal_d3(palette = "category20b")(15)),
...
#07、差异分析---- [FindMarkers]
{
  ...
  #预处理
  objs_9 <- objs_4
  conditon <- c(9,11); controls <- c(0,2)
  ...
  #
  #
  test <- mak__a[which(mak__a$p_val_adj <0.05 & mak__a$avg_log2FC > 0.58),]
...
{
  #目标：[在cluster中, Mhucs __vs__ Mcon]
  #目标：[在cluster中, Mhucp __vs__ Mcon]
  #预处理
  xx = 2; print(prex[xx]) #选择哪个处理组
  yy = 5 #选择哪个cluster
  ...
  mak__b <- Seurat::FindMarkers(object = objs_b, min.pct = 0.20,
                                ...
                                ident.1 = prex[xx], ident.2 = prex[1])
  volcan(f_Inputs = mak__b); p___01
  #write.csv(x = mak__b,file = paste0(path,"dumpROOM/p_c.Muscle.c4.TE.makers.csv"))
  #
  #
  #
  #
...
###临时--Venn图
{
  red1 <- read.csv(paste0(path,"dumpROOM/p_c.Bone.c6.TE.makers.csv"))
  red2 <- read.csv(paste0(path,"dumpROOM/p_c.Bone.c11.TE.makers.csv"))
  red3 <- read.csv(paste0(path,"dumpROOM/p_c.Muscle.c4.TE.makers.csv"))
  red1 <- red1[which(red1$avg_log2FC < -0.58 & red1$p_val_adj < 0.05),"X"]
  red2 <- red2[which(red2$avg_log2FC < -0.58 & red2$p_val_adj < 0.05),"X"]
  red3 <- red3[which(red3$avg_log2FC < -0.58 & red3$p_val_adj < 0.05),"X"]
  intersect(intersect(red1,red2),red3)
  #
  #
  tmp1 <- list()
  tmp1[["a"]] <- red1
  ...
  p_001 <- VennDiagram::venn.diagram(x = tmp1,
                                     ...
                                     category.names = c("Bone_c6","\n\nBone_c11",
                                                        "\n\nMuscle_c4"),
                                     cat.cex = 1.5,cat.dist = -0.05,
                                     fill=c("#eaafc8","#00b4db","grey70"),
                                     units = "cm", height = 12, width = 12)
  pdf(file = paste0(path,"PLOT/","venn.p_c",".pdf"), width = 8, height = 8)
  grid.draw(p_001)
  dev.off()
}
################################################################################
